# TalentFlow AI Recruitment Assistant

TalentFlow AI is a CrewAI-powered recruitment assistant that helps recruiting teams source candidates, evaluate fit, and deliver ranked recommendations faster and more consistently.

## Project Overview

This repository contains a mini-project implementation focused on high-impact recruiting workflows:
- candidate search based on job requirements
- automated candidate evaluation against role criteria
- ranked candidate recommendations for recruiter and hiring manager review

The system is human-in-the-loop by design. Agents support decisions, while recruiters and hiring managers retain final control.

## Problem Statement and Value Proposition

### Problem Statement
Recruiters and hiring managers spend significant time manually sourcing and evaluating candidates across fragmented tools. This leads to:
- slower hiring cycles
- inconsistent screening quality
- delayed shortlist decisions
- reduced scalability for high-volume recruitment

### Value Proposition
- **Speed:** reduce time to source and shortlist candidates.
- **Quality:** improve relevance and consistency of candidate matching.
- **Scalability:** support higher requisition volume without linear recruiter effort growth.
- **Trust:** provide explainable recommendations with clear rationale and approval checkpoints.

## Key Features

- **Candidate Search by Job Requirements**
  - structured role input and optional free-text parsing
  - filter-driven candidate retrieval
- **Automated Candidate Evaluation**
  - rubric-based scoring with criteria-level rationale
  - confidence indicators and manual override support
- **Ranked Candidate Recommendations**
  - prioritized shortlist with evidence-backed fit summaries
  - recruiter review before sharing with hiring manager
- **Ambiguity and Error Handling**
  - clarifying questions for incomplete requirements
  - recovery actions for no results, timeouts, and duplicate profiles

## Application Architecture Overview

### Runtime and Orchestration
- Runtime target: `crewai`
- Orchestration mode: sequential task flow for deterministic behavior
- Scope: mini-project baseline (no full ATS automation)

### Application Crew Agents
- **Researcher Agent**
  - Searches and sources candidate pool from job requirements.
- **Evaluator Agent**
  - Evaluates candidates against job criteria and computes fit scores.
- **Recommender Agent**
  - Produces ranked candidate shortlist with concise rationale.

### Human Roles in the Loop
- **Recruiter**
  - Inputs role requirements, tunes filters/rubric, approves shortlist.
- **Hiring Manager**
  - Reviews ranked recommendations and supports final selection decisions.
...
Create `.env` (or `.env.example` if present) and set:
```env
AAMAD_TARGET_RUNTIME=crewai
```

Add any API keys and service settings required by your implementation module.

### 5. Review product artifacts
- MRD: `project-context/1.define/mrd.md`
- PRD: `project-context/1.define/prd.md`

### 6. Build in module order
Follow module boundaries and complete each independently:
...
│   ├── rules/             # Project and runtime rules
│   └── templates/         # MRD/PRD/SAD templates
├── project-context/
│   ├── 1.define/
│   │   ├── mrd.md         # Market Requirements Document
│   │   └── prd.md         # Product Requirements Document
│   ├── 2.build/           # Build-phase artifacts (setup/backend/frontend/integration/qa)
│   └── 3.deliver/         # Delivery/deployment artifacts
├── AGENTS.md              # Agent framework summary
...
- Validate PRD assumptions against your real recruiting workflow and data.
- Implement module 1 artifacts (`config/agents.yaml`, `config/tasks.yaml`, runtime entrypoint).
- Implement candidate search/evaluation/recommendation workflows in module 2.
- Build recruiter-facing interaction experience in module 3.
- Execute end-to-end validation and KPI checks in module 4.
- Track outcomes against PRD success metrics:
